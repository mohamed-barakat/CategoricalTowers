    ...
    AddIsWellDefinedForMorphisms( Prop,
      function ( Prop, mor )
        ...
        return ForAll( datum, L -> IsBigInt( L[1] ) and L[1] >= 0 ) and
               ...
               ForAll( datum, L -> IsInt( L[2] ) and L[2] in [ 0, 1, 2 ] ) and
               ForAll( datum, L -> ( ( not L[1] = L[3] ) or L[2] = 0 ) and ( ( not L[2] = 0 ) or L[1] = L[3] ) ) and
...
InstallMethod( PreSheafEncodingPathCategory,
  ...
  function ( q )
    ...
    obj_func :=
      function( i )
        local t, s, fiber_product, si, ti, j;
    ...
    morphism_func :=
      function( source, mor, target )
        ...
        return mor_func( triple[1], triple[2], triple[3] );
