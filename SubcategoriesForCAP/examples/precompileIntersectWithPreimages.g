...
int := IntersectWithPreimagesWithGivenObjects( sFinSets, PAxB, map, PA );
Assert( 0, IsWellDefined( int ) );
Display( int );

int_compiled_notoptimized := intersection_compiled_notoptimized( sFinSets, PAxB, map, PA );
Assert( 0, int = int_compiled_notoptimized );

int_compiled := intersection_compiled( sFinSets, PAxB, map, PA );
Assert( 0, int = int_compiled );
