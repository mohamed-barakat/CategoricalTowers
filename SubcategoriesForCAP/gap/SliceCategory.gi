  ...
  function( B, over_tensor_unit, name, category_filter, category_object_filter, category_morphism_filter, object_constructor, object_datum )
    ...
    if CanCompute( C, "PowerObject" ) and
       ...
       CanCompute( C, "PLeftTransposeMorphismWithGivenRange" ) then
        ...
        ##
        AddPowerObjectRightEvaluationMorphismWithGivenObjects( Slice_over_B,
          function( cat, MxPM, M, Omega )
            local C, f;
            ...
            return MorphismConstructor( cat,
                           MxPM,
                           RelativePowerObjectRightEvaluationMorphism( C, f ),
                           Omega );
        ...
        ##
        AddPRightTransposeMorphismWithGivenRange( Slice_over_B,
          function( cat, M, N, f, PM )
            local C, Omega, B, m, n, ff, pr1, chi, iota, emb, cmp, h, Ms, Ns, PMs, k, kn, e, g;
            ...
            ## P(Ms)
            PMs := PowerObject( C, Ms );
            
            ## k: Ns → P(Ms)
            k := PRightTransposeMorphismWithGivenRange( C,
                         Ms,
                         Ns,
                         h,
                         PMs );
            ...
            ## e: P_m(Ms) ↪ P(Ms) × B
            e := EmbeddingOfRelativePowerObject( C, m );
            
            ## ⟨k,n⟩: Ns → P(Ms) × B
            kn := UniversalMorphismIntoDirectProductWithGivenDirectProduct( C,
                          [ PMs, B ],
                          Ns,
                          [ k, n ],
                          Range( e ) );
