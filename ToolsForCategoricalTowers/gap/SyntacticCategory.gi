        ...
        if "PreComposeList" in list_of_operations_to_install then
            ...
            AddPreComposeList( syntactic_cat,
              function( syntactic_cat, source, list_of_mors, target )
                local l, extract_argument, condition, p, objects_pre, objects_pst, i, datum, D, n, tau;
                ...
                if l > 0 and PairOfOperationNameAndArguments( list_of_mors[l] )[1] = "UniversalMorphismIntoTerminalObjectWithGivenTerminalObject" then
                    
                    Info( InfoSyntacticCategory, 2,
                          "PreComposeList( source, [ …, object → 𝟏 ], 𝟏 ) ~> source → 𝟏",
                          "        (l = ", String( l ), ")" );
                    
                    return UniversalMorphismIntoTerminalObjectWithGivenTerminalObject( syntactic_cat, source, target );
                ...
                if IsInt( p ) then
                    ...
                    Info( InfoSyntacticCategory, 2,
                          "PreComposeList( [ …, ⟨ …, τᵢ, … ⟩, πᵢ, … ] ) ~> PreComposeList( [ …, τᵢ, … ] )",
                          "        (p = ", String( p ), ", i = ", String( i ), ")" );
                    
                    return PreComposeList( source, list_of_mors, target );
                ...
                if l > 1 and PairOfOperationNameAndArguments( list_of_mors[l] )[1] = "UniversalMorphismIntoDirectProductWithGivenDirectProduct" then
                    ...
                    Info( InfoSyntacticCategory, 2,
                          "PreComposeList( [ …, ⟨ τ₁, …, τₙ ⟩ ] ) ~> ⟨ PreComposeList( [ …, τ₁ ] ), …, PreComposeList( [ …, τₙ ] ) ⟩",
        ...
        if "UniversalMorphismIntoDirectProductWithGivenDirectProduct" in list_of_operations_to_install then
            ...
            AddUniversalMorphismIntoDirectProductWithGivenDirectProduct( syntactic_cat,
              function( syntactic_cat, D, T, tau, P )
                local n, data, list_of_mors, lengths, l, tail;
                ...
                if ForAll( [ 1 .. n ], i ->
                           data[i][1] = "ProjectionInFactorOfDirectProductWithGivenDirectProduct" and
                            ...
                            if ForAll( [ 1 .. n ], i ->
                                ...
                                if n = Length( tail[1][2][2] ) and
                                   ForAll( [ 1 .. n ], i -> i = tail[i][2][3] ) then
                                    ...
                                    Info( InfoSyntacticCategory, 2,
                                          "⟨ PreComposeList( [ …, π₁ ] ), …, PreComposeList( [ …, πₙ ] ) ⟩ ~> PreComposeList( [ … ] )",
                                          "        (l = ", String( l ), ", n = ", String( n ), ")" );
