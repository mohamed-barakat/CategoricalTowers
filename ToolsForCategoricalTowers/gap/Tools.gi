...
InstallGlobalFunction( CAP_INTERNAL_CORRESPONDING_WITH_GIVEN_OBJECTS_METHOD,
  function( name_of_cap_operation, list_of_installed_operations )
    ...
    if IsList( info.with_given_without_given_name_pair ) and
       Length( info.with_given_without_given_name_pair ) = 2 and
       name_of_cap_operation = info.with_given_without_given_name_pair[1] and
       not IsBound( CAP_INTERNAL_METHOD_NAME_RECORD.(info.with_given_without_given_name_pair[2]).with_given_object_name ) then
        
        return [ info.with_given_without_given_name_pair[2] ];
        
    fi;
    
    if not ( IsList( info.with_given_without_given_name_pair ) and
             Length( info.with_given_without_given_name_pair ) = 2 and
             name_of_cap_operation = info.with_given_without_given_name_pair[1] and
