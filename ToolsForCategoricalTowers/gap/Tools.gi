...
InstallMethod( SyntacticCategoryInDoctrines,
  ...
  function( doctrine_names )
    local additional_operations, minimal, compare, options, name, ring, all_operations, with_given;
    ...
    name := CAP_INTERNAL_RETURN_OPTION_OR_DEFAULT( "name", Concatenation( "SyntacticCategoryInDoctrines( ", String( doctrine_names ), " )" ) );
    
    options.name := name;
    
    options.properties := Difference( doctrine_names, [ "EveryCategory" ] );
    
    ring := ValueOption( "ring" );
    
    if IsRing( ring ) then
        options.commutative_ring_of_linear_category := ring;
    fi;
    ...
    with_given := ValueOption( "with_given" );
    
    if IsIdenticalObj( with_given, true ) then
