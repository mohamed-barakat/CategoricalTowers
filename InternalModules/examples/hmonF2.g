...
#! Rows( GF(2)[c1..168,s1..9] )
ComonSmat := CategoryOfComonoids( Smat );;
#! CategoryOfComonoids( Rows( GF(2)[c1..168,s1..9] ) )
BimonSmat := CategoryOfBimonoids2( Smat );
#! CategoryOfBimonoids2( Rows( GF(2)[c1..168,s1..9] ) )
HmonSmat := CategoryOfHopfMonoids( Smat : reverse := true );
#! CategoryOfHopfMonoids( Rows( GF(2)[c1..168,s1..9] ) )
...
#mat := UnderlyingColumn( gamma_small );
#BasisOfRows( mat ); # 10 days, 42 GB
...
#gamma_smallest := EmbedInSmallerAmbientSpace( gamma_small ); # 9days, 262 GB
...
Assert( 0, Length( radicals ) = 2^9 );
List( radicals, NrRows );

bas := List( [ 1 .. 2^9 ], func_bas );

break;

#for i in [ 1 .. 2^9 ] do SaveHomalgMatrixToFile( Concatenation( "radicals_of_linear_equations_F2/radicals_of_linear_equations_", String( i ) ), radicals[i] ); od;
radicals := [ ]; for i in [ 1 .. 2^9 ] do radicals[i] := LoadHomalgMatrixFromFile( Concatenation( "radicals_of_linear_equations_F2/radicals_of_linear_equations_", String( i ) ), 30, 1, T ); od; radicals := List( radicals, mat -> CertainRows( mat, NonZeroRows( mat ) ) );
