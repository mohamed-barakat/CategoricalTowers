       ...
       CanCompute( C, "ColiftAlongEpimorphism" ) then
        ...
        AddIsColiftableAlongEpimorphism( UCm,
          function( UCm, pi, phi )
            ...
            # pi_maps need to be an epimorphism
            if not ForAll( [ 1 .. l ], o ->
            ...
            # If pi_maps_o_i = pi_maps_p_j then phi_maps_o_i = phi_maps_p_j
            if not ForAll( [ 1 .. l ], o ->
                           ForAll( [ 1 .. m_source[o] ], i ->
                                   ForAll( [ 1 .. l ], p ->
                                           ForAll( [ 1 .. m_source[p] ], j ->
                                                   not ( map_pi[o][1][i] = map_pi[p][1][j] and map_pi[o][2][i] = map_pi[p][2][j] ) or
                                                   ( map_phi[o][1][i] = map_phi[p][1][j] and map_phi[o][2][i] = map_phi[p][2][j] )
                                                  ) ) ) )
            then
              return false;
            fi;
            ...
            preim := List( [ 1 .. l ], o ->
                           List( [ 1 .. m_target[o] ], i ->
                                 Concatenation( List( preim_o[o][i], e ->
                                      List( Filtered( [ 1 .. m_source[e] ], j -> 1 + map_pi[e][1][j] = o and 1 + map_pi[e][2][j] = i  ), f -> [ e, f ] ) ) ) ) );
    ...
    if CanCompute( C, "ColiftAlongEpimorphism" ) then
        ...
        AddColiftAlongEpimorphism( UCm,
          function( UCm, pi, phi )
            ...
            map := List( [ 1 .. l ], o ->
                         [ List( [ 1 .. m_target[o] ], i -> dphi[1][preim_o[o][i]][1][preim_i[o][i]] ),
